...
**Server:** college-scorecard-mcp-server
**Version:** 0.1.9
...
- **Use `ctx.log`** for request-scoped logging. No `console` calls. Dual-sink: Pino **and** `notifications/message` to the client — treat it as client-visible.
...
This server declares `sessionMode: 'stateless'` in `src/index.ts`: no tool calls `ctx.requestInput`, so no request depends on a live session.
...
| `bun run release:github` | Create the GitHub release for the current tag |
| `bun run publish-mcp` | Publish to the MCP registry via `mcp-publisher` |
